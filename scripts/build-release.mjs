...
const tarball = resolve(root, 'dist', packageInfo.filename)
const materials = await writeReleaseMaterials({ root, tarball, channel, packageFiles: packageInfo.files?.map(file => `package/${file.path ?? file}`) ?? [], runtimeManifestPath: 'runtime-manifests/postgresql-18.6-darwin.json', evidencePath, agentFixturePath: value('--agent-fixture') ?? 'docs/compatibility/0.1.0-rc.6-agent-runtime-fixture.json', distribution })
