...
**下一步：T12、T14、T18、T25 已完成；T16 的双架构 PostgreSQL runtime 已通过公开 GitHub Release 和 attestation，T19 矩阵、T20 生产 Bundle 接线与 T21 材料审计已刷新。T21 的 DbGate 运行时已收敛为 PostgreSQL-only 并移除 `xlsx`/Excel 插件路径，官方 npm registry 复核为 0 critical/0 high/8 moderate；当前 T21 只剩 Bundle release-candidate 的完整生产 provenance、签名材料和 clean Profile 验收。T22 的 arm64 干净 Profile 生命周期证据已刷新，但完整生产组合、真实模型和签名 Bundle 仍未满足前不勾选。T19/T20 的隔离 E2E、Bundle 和本地材料回归已刷新，真实生产组合仍需受保护模型和发布材料。阻塞项的根因、解除顺序和外部输入已写入 [发布阻塞处置方案](docs/operations/release-blockers.md)，后续按该顺序推进；不把 LangGraph spike 依赖并入 Bundle 默认路径。**
...
- [ ] **T21 ☐ 发布材料｜runtime 已公开发布，Bundle release-candidate 门禁待补齐｜R18**
...
- 2026-09-16 本地材料刷新：按当前 `package-lock.json` 重新生成 Bundle 0.1.0 的 checksum/SBOM/materials，归档 256 项，SHA-256 `26620a76c2d1f33aa324d33d8b4cf6ce86dd41f5e611eea9435aff0b489fb8d2`，SBOM SHA-256 `df5144dde9ebef11cfcc03cd8d0098a5cf44a7743e48e671c3f848f182323fad`。`audit-release-materials` 的 archive-checksum-sbom、许可证、DbGate 版本与 PostgreSQL-only profile 均通过；审计证据 `.backend-team/artifacts/release-materials-t21-20260916/audit.json`。PostgreSQL manifest 仍为 `pending-native-build`，签名状态 `not-attested` 且无稳定 HTTPS 地址，T21 继续保持未勾选。
  ...
  - 2026-09-21 GitHub Actions 首次复验发现两架构都在 `npm ci` 阶段失败：npm 自动选择了 `@deepseek-ai/dsh-client-locale@0.1.0-rc.8`，与 Bundle/宿主要求的 DSH rc.6 peer 树冲突。已在 PostgreSQL runtime 和 release workflow 固定 `npm ci --ignore-scripts --legacy-peer-deps`，并用干净源码归档、Node 24.19.0 复现安装和完整 workspace build 通过。
  - 2026-09-21 GitHub Actions Run `35558685646` 已通过（7m32s）：macOS arm64 与 x64 两个 job 均完成 Node 24 安装、workspace build、PostgreSQL 生命周期 smoke、证据归档和 GitHub attestation；attestation 分别为 `48835525`（arm64）和 `48835943`（x64），artifact digest 分别为 `sha256:c3bebdd5b283a978c0cf781f086cb1ae8462600bda1bbdca05c28dd967a4d9b5` 与 `sha256:56f240776c0fb909e4023864efa906dcb4a16c7cb217bbb356659a68f0079daf`。attestation 页面同时给出归档 subject 摘要：arm64 `373116ccd2ed11113f5b0a8f2807d8989d7c11aaee7456260b040cb9c0178e33`，x64 `1c1cd3ac2ad2504351cbedae1efd283ad7d975f59c0a4e5574c2941dc7c8fffd`。仍需从同一 run 下载归档并以 `gh attestation verify` 核对本地文件摘要，发布到稳定 HTTPS URL 后生成 verified manifest；T21 暂不勾选。
  - 2026-09-21 GitHub Actions Run `35558685646` 已通过（7m32s）：macOS arm64 与 x64 两个 job 均完成 Node 24 安装、workspace build、PostgreSQL 生命周期 smoke、证据归档和 GitHub attestation；attestation 分别为 `48835525`（arm64）和 `48835943`（x64），artifact digest 分别为 `sha256:c3bebdd5b283a978c0cf781f086cb1ae8462600bda1bbdca05c28dd967a4d9b5` 与 `sha256:56f240776c0fb909e4023864efa906dcb4a16c7cb217bbb356659a68f0079daf`。attestation 页面同时给出归档 subject 摘要：arm64 `373116ccd2ed11113f5b0a8f2807d8989d7c11aaee7456260b040cb9c0178e33`，x64 `1c1cd3ac2ad2504351cbedae1efd283ad7d975f59c0a4e5574c2941dc7c8fffd`；后续正式发布使用新的 workflow dispatch run。
  - 2026-09-21 正式 runtime 发布：Run `35565633720` 的 arm64/x64 构建、attestation 和 `publish-runtime` job 全部成功（10m10s）。Release [v0.1.0-rc.1](https://github.com/zhuangdize/dsh-backend-team/releases/tag/v0.1.0-rc.1) 提供两个归档、两个 attestation JSON、两个 smoke、源码 manifest、`SHA256SUMS` 与 verified manifest；公开 manifest 已回写到 `runtime-manifests/postgresql-18.6-darwin.json`，`assertRuntimeManifest` 远程复核通过。T21 仍未勾选，因为 Bundle 的完整 release-evidence、签名 distribution 和签名 release-candidate 还未通过。
  - 2026-09-21 发布流程修复：`.github/workflows/release-bundle.yml` 现在要求 `evidence_path`、`release_tag` 与已审核的 `agent_fixture_path`，在同一归档上依次执行 clean checkout、先构建 workspace 产物再跑全量检查、Bundle 打包、GitHub Actions attestation、摘要绑定的签名 distribution、`verify-release.mjs`、`audit-release-materials.mjs` 和 GitHub Release 发布；`scripts/build-release.mjs` 支持对已打包归档二次写入签名材料，并在干净 checkout 自动创建 `dist/`。用临时 clean checkout、Node 24 和结构化测试材料跑通“未签名构建 → 签名材料回写 → release-candidate verify”链路。该修复消除了发布自动化缺口，但没有把隔离材料升级为真实生产 evidence，T21 仍待四项生产 gate、verified Agent fixture、签名候选包和 T22 干净 Profile 验收。
